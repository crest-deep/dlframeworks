...
def main():
    ...
    # --------
    # --------
    import rmsprop_warmup
    optimizer = chainermn.create_multi_node_optimizer(
        rmsprop_warmup.RMSpropWarmup(), comm)

    #optimizer = chainermn.create_multi_node_optimizer(
    #    chainer.optimizers.RMSprop(), comm)
    ...
    trainer.extend(rmsprop_warmup.RMSpropWarmupScheduler(
    ...
    if comm.rank == 0:
        ...
        trainer.extend(extensions.observe_value(
            'alpha_sgd', lambda t: t.updater.get_optimizer('main').alpha_sgd),
            trigger=log_interval)
        trainer.extend(extensions.observe_value(
            'alpha_rmsprop',
            lambda t: t.updater.get_optimizer('main').alpha_rmsprop),
            trigger=log_interval)
        trainer.extend(extensions.PrintReport([
            'epoch', 'iteration', 'main/loss', 'validation/main/loss',
            'main/accuracy', 'validation/main/accuracy', 'lr', 'alpha_sgd',
            'alpha_rmsprop'
        ]), trigger=log_interval)
